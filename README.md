...
- **Don't register before the registry loads its own tools.** `ensure_tools_loaded!` skips the `tool_paths` pass once the registry is non-empty, so registering first makes every file-defined tool silently disappear from `tools/list`. Producers run *after* that pass, so the ordering is not yours to get right.
